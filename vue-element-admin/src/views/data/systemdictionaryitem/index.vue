<template>
  <div style="height: calc(100vh - 84px);">
    <el-card class="box-card" style="height: 100%">
      ...
      <el-table v-loading="loading" border :data="tableData" style="width: 100%" max-height="690"
                @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="100" align="center"/>
        <el-table-column type="index" width="50" label="序号" align='center'/>
        <el-table-column prop="systemdictionary.name" label="数据字典类型"/>
        <el-table-column prop="name" label="数据字典明细"/>
        <el-table-column prop="requence" label="明细编号"/>
        <el-table-column prop="intro" label="详情"/>
      ...
      <el-dialog title="信息管理" :visible.sync="dialogFormVisible" :close-on-click-modal="false" width="35%">
        <el-form ref="addForm" :model="addForm" label-width="100px" :rules="rules">
          ...
          <el-form-item label="字典类型" prop="systemdictionary.name">
            <el-select v-model="addForm.systemdictionary.id" placeholder="请选择套餐">
              <!--key:id -->
              <el-option
                v-for="item in systemdictionarys"
                :key="item.id"
                :label="item.name"
                :value="item.id"
              />
          ...
          <el-form-item label="明细编号" prop="requence">
            <el-input v-model="addForm.requence" autocomplete="off"/>
          </el-form-item>
          <el-form-item label="详情" prop="intro">
...
<script>
  export default {
    ...
    methods: {
      // 显示添加弹窗
      handleShowAddDialog() {
        this.$http.get('/systemdictionary/findAll').then(res => {
          this.systemdictionarys = res.data.data;
      ...
      submitForm(formName) { /* 确认保存*/
        ...
        refs[formName].validate((valid) => {
          ...
          let url = '/systemdictionaryitem/save'
          if (this.addForm.id) {
      ...
      handleBatchRemove() {
        ...
        http.post('/systemdictionaryitem/batchDelete', param).then(res => {
          if (res.data.success) {
      ...
      handleRemove(row) {
        var http = this.$http
        var notify = this.$notify;
        http.delete('/systemdictionaryitem/delete/' + row.id).then(res => {
          if (res.data.success) {
            notify({title: '删除成功', message: '恭喜你，你已经成功删除', type: 'success', offset: 100})
            this.loadListData()
          } else {
      ...
      loadListData() {
        ...
        var http = this.$http;
        var message = this.$message
        http.post('/systemdictionaryitem/selectForPage', param).then(res => {
          if (res.data.success) {

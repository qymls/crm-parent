  ...
  <div style="height: calc(100vh - 84px);">
    <el-card class="box-card" style="height: 100%">
        ...
        <el-col :span="21" class="toolbar">
          ...
          >
            <el-form-item label="用户名">
              <el-input v-model="searchForm.name" placeholder="字典名称" clearable/>
            </el-form-item>
      ...
      <el-table v-loading="loading" border :data="tableData" style="width: 100%" max-height="690"
        ...
        <el-table-column prop="intro" label="字典介绍"/>
      ...
      <el-dialog title="信息管理" :visible.sync="dialogFormVisible" :close-on-click-modal="false" width="35%">
        <el-form ref="addForm" :model="addForm" label-width="100px" :rules="rules">
          ...
          <el-form-item label="字典介绍" prop="intro">
          ...
          <el-form-item label="租户" prop="tenant.name">
            <el-select v-model="addForm.tenant.id" placeholder="请选择套餐">
              <!--key:id -->
              <el-option
  ...
  export default {
    ...
    methods: {
      ...
      handleBatchRemove() {
        ...
        http.post('/systemdictionary/batchDelete', param).then(res => {
          if (res.data.success) {
      ...
      handleRemove(row) {
        var http = this.$http
        var notify = this.$notify;
        http.delete('/systemdictionary/delete/' + row.id).then(res => {
          if (res.data.success) {
            notify({title: '删除成功', message: '恭喜你，你已经成功删除', type: 'success', offset: 100})
            this.loadListData()
          } else {
      ...
      loadListData() {
        ...
        var http = this.$http;
        var message = this.$message
        http.post('/systemdictionary/selectForPage', param).then(res => {
          if (res.data.success) {
